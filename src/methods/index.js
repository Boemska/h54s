...
module.exports.call = function (sasProgram, dataObj, callback, params) {
	...
	// TODO add headers and set csrf in header as it's done on managedRequest
	var _headers = {}
	if (csrf) {
		_headers[defaultCsrfHeader] = csrf
	}
	...
	this._ajax.post(this.url, params, _headers, this.useMultipartFormData).success(function (res) {
		if (self._utils.needToLogin.call(self, res)) {
			...
			if (!dbg) {
				try {
					...
					if (e instanceof SyntaxError) {
						if (retryCount < self.maxXhrRetries) {
							...
							self._ajax.post(self.url, params, _headers, self.useMultipartFormData).success(this.success).error(this.error);
							retryCount++;
				...
				try {
					// Depending on isViya proper logic will be applied in parseDebugRes()
		...
		if (res.status == 449 || (res.status == 403 && (res.responseText.includes('_csrf') || res.getResponseHeader('X-Forbidden-Reason') === 'CSRF') && (_csrf = res.getResponseHeader(res.getResponseHeader('X-CSRF-HEADER'))))) {
			self.csrf = _csrf;
			if (retryCount < self.maxXhrRetries) {
				if (res.status === 403 && _csrf) {
					_headers[defaultCsrfHeader] = _csrf
				}
				self._ajax.post(self.url, params, _headers, true).success(this.success).error(this.error);
				retryCount++;
				logs.addApplicationLog("Retrying #" + retryCount, sasProgram);
			} else {
				self._utils.parseErrorResponse(res.responseText, sasProgram);
...
module.exports.managedRequest = function (callMethod = 'get', _url, options = {
	...
	// Check if url is sent with hostUrl
	// When dealing with rest api sometimes full url is returned sometimes not
	let url = _url
	if (!_url.startsWith(self.hostUrl)) {
	...
	this._ajax[callMethod](url, params, _headers, useMultipartFormData).success(function (res) {
		if (self._utils.needToLogin.call(self, res)) {
			//remember the call for latter use
			self._customPendingCalls.push({
				callMethod,
		...
		if (res.status == 449 || (res.status == 403 && (res.responseText.includes('_csrf') || res.getResponseHeader('X-Forbidden-Reason') === 'CSRF') && (_csrf = res.getResponseHeader(res.getResponseHeader('X-CSRF-HEADER'))))) {
			...
			self.csrf = _csrf
			if (retryCount < self.maxXhrRetries) {
				var __headers = Object.assign({}, _headers, {'X-CSRF-TOKEN': _csrf})
				self._ajax[callMethod](url, params, __headers, useMultipartFormData).success(this.success).error(this.error);
				// self._ajax[callMethod](url, params).success(this.success).error(this.error);
				retryCount++;
			} else {
